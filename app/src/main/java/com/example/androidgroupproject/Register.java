...
//      initialize the calendar on click
    ...
    private void userProfile(String email, String password) {
        ...
        if(user != null){
            ...
            // Add a new document with a generated ID
            db.collection("users").document(email).collection("Profile").document("CurrentItem").set(equipment)
                    .addOnSuccessListener(new OnSuccessListener<Void>() {
                        @Override
                        public void onSuccess(Void aVoid) {
            ...
            try {
                StorageReference mstore_profile  = store_image.child(uphotos)
                        .child(email)
                        .child(profile)
                        .child(user_name.getText().toString() +".jpg");
